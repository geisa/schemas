...
NANOPB_METERED_GEN = \
	...
	$(NANOPBDIR)/metered_quantities.pb.h
NANOPB_METERED_STAMP = $(NANOPBDIR)/.metered_quantities.nanopb.stamp
NANOPB_METERED_EXAMPLES = \
	$(EXAMPLESDIR)/metered_quantities_write_example_c \
	$(EXAMPLESDIR)/metered_quantities_read_example_c
...
$(NANOPB_METERED_STAMP): $(NANOPB_METERED_PROTO) $(NANOPB_METERED_OPTIONS)
	$(call NANOPB_GENERATE,$(NANOPB_METERED_PROTO),$(NANOPB_METERED_GEN))
	@touch $@
...
$(EXAMPLESDIR)/metered_quantities_write_example_c: examples/metered_quantities_write_example.c examples/helpers/metered_quantities_example_utils.h $(NANOPB_METERED_STAMP)
	@mkdir -p $(EXAMPLESDIR)
	@if [ -z "$(NANOPB_RUNTIME_DIR_EFFECTIVE)" ] || [ ! -f "$(NANOPB_RUNTIME_DIR_EFFECTIVE)/pb_encode.c" ] || [ ! -f "$(NANOPB_RUNTIME_DIR_EFFECTIVE)/pb_decode.c" ] || [ ! -f "$(NANOPB_RUNTIME_DIR_EFFECTIVE)/pb_common.c" ]; then \
	  echo "NANOPB_DIR or NANOPB_RUNTIME_DIR must point to a nanopb source tree containing pb_common.c, pb_encode.c, and pb_decode.c."; \
	  exit 1; \
...
$(EXAMPLESDIR)/metered_quantities_read_example_c: examples/metered_quantities_read_example.c examples/helpers/metered_quantities_example_utils.h $(NANOPB_METERED_STAMP)
	@mkdir -p $(EXAMPLESDIR)
	@if [ -z "$(NANOPB_RUNTIME_DIR_EFFECTIVE)" ] || [ ! -f "$(NANOPB_RUNTIME_DIR_EFFECTIVE)/pb_encode.c" ] || [ ! -f "$(NANOPB_RUNTIME_DIR_EFFECTIVE)/pb_decode.c" ] || [ ! -f "$(NANOPB_RUNTIME_DIR_EFFECTIVE)/pb_common.c" ]; then \
	  echo "NANOPB_DIR or NANOPB_RUNTIME_DIR must point to a nanopb source tree containing pb_common.c, pb_encode.c, and pb_decode.c."; \
	  exit 1; \
